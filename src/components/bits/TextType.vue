...
const executeTypingAnimation = () => {
  ...
  if (isDeleting.value) {
    if (displayedText.value === '') {
      ...
      const sentenceText = textArray.value[currentTextIndex.value]
      if (sentenceText) {
        props.onSentenceComplete?.(sentenceText, currentTextIndex.value)
      }
    ...
    if (currentCharIndex.value < processedText.length) {
      ...
      // 打字完成
      const isLastText = currentTextIndex.value === textArray.value.length - 1

      // 如果是最后一个文本且不循环，触发完成回调
      if (isLastText && !props.loop) {
        const sentenceText = textArray.value[currentTextIndex.value]
        if (sentenceText) {
          props.onSentenceComplete?.(sentenceText, currentTextIndex.value)
        }
